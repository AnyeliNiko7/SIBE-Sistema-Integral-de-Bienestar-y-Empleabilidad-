...
// Verifica que el usuario haya iniciado sesión y que pertenezca al área de Psicología
if (!isset($_SESSION['id']) || $_SESSION['area'] !== 'Psicología') {
    header("location: ../../index.php"); // Redirige al login si no cumple
    exit(); // Detiene la ejecución
}

// Incluye la conexión a la base de datos - CORREGIR RUTA si es necesario
...
switch ($agrupacion) {
    case 'mes':
        $groupBy = "DATE_FORMAT(rp.fecha, '%Y-%m')";
        $format = '%m-%Y';
        $selectField = "DATE_FORMAT(rp.fecha, '%Y-%m') AS agrupacion";
        // Filtro por mes
        $filtro_fecha = "AND MONTH(rp.fecha) = '$mes_seleccionado' AND YEAR(rp.fecha) = '$anio_seleccionado'";
        break;
    case 'anio':
        $groupBy = "YEAR(rp.fecha)";
        $format = '%Y';
        $selectField = "YEAR(rp.fecha) AS agrupacion";
        // Filtro por año
        $filtro_fecha = "AND YEAR(rp.fecha) = '$anio_seleccionado'";
        break;
    default:
        $groupBy = "DATE(rp.fecha)";
        $format = '%d-%m-%Y';
        $selectField = "DATE(rp.fecha) AS agrupacion";
        // Filtro por día
        $filtro_fecha = "AND DATE(rp.fecha) = '$anio_seleccionado-$mes_seleccionado-$dia_seleccionado'";
}

// Consulta SQL que obtiene los registros activos de psicología
$query = "SELECT $selectField, rp.fecha, rp.id, rp.dni, rp.apellidos_nombres, 
                 c.nombre AS carrera, s.nombre AS semestre, t.nombre AS turno, rp.motivo_consulta 
...
<head>
    ...
    <title>Gestión de Información Psicológica</title>
...
<body>
    ...
    <main class="home-section">
        <section>
            <h2>Visualización de registros de psicología</h2>
                        ...
                        if ($resultado->num_rows > 0) {
                            ...
                            while ($fila = $resultado->fetch_assoc()):
                                ...
                                <tr>
                                    ...
                                    <td><?= htmlspecialchars($fila['motivo_consulta']) ?></td>
                                </tr>
                            <?php endwhile;
                        } else {
    ...
    <script>
        ...
        document.getElementById('btnDescargarPDF').addEventListener('click', () => {
            ...
            const tableConfig = {
                head: [["Nº", "DNI", "Apellidos y Nombres", "Carrera", "Semestre", "Turno", "Motivo de Consulta"]],
                body: [],
                theme: "grid",
                styles: {
            ...
            doc.save('tabla_informacion_psicologia.pdf');
        });
        ...
        document.getElementById('btnDescargarExcel').addEventListener('click', () => {
            ...
            XLSX.utils.book_append_sheet(wb, ws, 'Registros Psicología');

            const nombreArchivo = `Registros_Psicologia.xlsx`;
